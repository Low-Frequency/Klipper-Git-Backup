...
github_menu() {
  ...
  while true; do
    ...
    b | B)
      ...
      ;;
    1)
      read -r -p "$(echo -e "${PURPLE}Please enter your Git username: ${NC}")" GITHUB_USER
      success_msg "Username set to ${GITHUB_USER}"
      UNSAVED_CHANGES=1
      ;;
    2)
      read -r -p "$(echo -e "${PURPLE}Please enter your mail address: ${NC}")" GITHUB_MAIL
      success_msg "Mail set to ${GITHUB_MAIL}"
      UNSAVED_CHANGES=1
      ;;
    3)
      read -r -p "$(echo -e "${PURPLE}Please enter the default branch you want to use: ${NC}")" GITHUB_BRANCH
