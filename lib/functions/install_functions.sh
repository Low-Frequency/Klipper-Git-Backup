...
setup_ssh() {
  ...
  if [[ -f "$HOME/.ssh/github_id_rsa" ]]; then
    ...
    while true; do
      read -r -p "$(echo -e "${CYAN}Did you already add this key to your Git account? ${NC}")" KEY_ALREADY_ADDED
...
install() {
  ...
  for i in "${!REPO_LIST[@]}"; do
    if [[ -d "${CONFIG_FOLDER_LIST[$i]}/.git" ]]; then
